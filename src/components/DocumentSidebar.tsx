...
export function DocumentSidebar({ mobile, open, onOpenChange }: DocumentSidebarProps) {
  ...
  useEffect(() => {
    const fetchMyDocs = async () => {
      try {
        const res = await fetch("https://15.207.221.31:1234/api/my-docs", {
          headers: {
  ...
  const createDoc = async () => {
    try {
      ...
      const res = await fetch("https://15.207.221.31:1234/api/create-doc", {
        method: "POST",
        headers: {
  ...
  const updateDocTitle = async (documentId: string, newTitle: string) => {
    try {
      ...
      const res = await fetch(`https://15.207.221.31:1234/api/update-doc/${documentId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
